...
description: Use when a raw feature idea (from brainstorms or conversation) needs structured refinement before it can be planned with feature-3-plan.
---
...
Creates `docs/features/needs_refinement/FEATURE-NAME.md` with the full refinement schema. If the idea comes from `ideas.md`, moves it to a `## Promoted` section there.
...
3. **Fill schema** interactively with the user — see File Schema below for all fields and sections.
...
7. Set status to `needs-review` once all fields are filled. Never set status to `ready` — only `feature-3-plan` does that.
