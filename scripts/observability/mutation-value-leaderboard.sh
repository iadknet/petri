...
fetch_json "/v3/simulation/status" | "${JQ_BIN}" \
	...
	| [
		...
		| {
			...
			helpful_total: (.value.helpful_total // 0),
			neutral_total: (.value.neutral_total // 0),
			detrimental_total: (.value.detrimental_total // 0),
			viability_score_sum: (.value.viability_score_sum // 0),
			viability_score_delta_sum: (.value.viability_score_delta_sum // 0),
			action_attempted_total: (.value.action_attempted_total // 0),
			invalid_action_total: (.value.invalid_action_total // 0),
			blocked_move_total: (.value.blocked_move_total // 0),
			invalid_reproduce_total: (.value.invalid_reproduce_total // 0),
			confidence_high_total: (.value.confidence_high_total // 0)
		}
	] as $rows
	...
	| ($rows | map(.helpful_total) | add // 0) as $helpful_total
	| ($rows | map(.neutral_total) | add // 0) as $neutral_total
	| ($rows | map(.detrimental_total) | add // 0) as $detrimental_total
	| ($rows | map(.viability_score_sum) | add // 0) as $viability_score_sum
	| (if $carrier_total == 0 then {
		mean_survival_ticks: 0,
		mean_offspring_spawned: 0,
		mean_final_energy: 0,
		mean_viability_score: 0
	   } else {
		...
		mean_viability_score: ($viability_score_sum / $carrier_total)
	   } end) as $global
	| {
		tick,
		population,
		global: {
			...
			mean_viability_score: $global.mean_viability_score,
			helpful_total: $helpful_total,
			neutral_total: $neutral_total,
			detrimental_total: $detrimental_total
		},
		leaderboard: (
			$rows
			...
			| map(
				. + {
					...
					mean_viability_score: (
						if .carriers_observed_total == 0 then 0
						else (.viability_score_sum / .carriers_observed_total)
						end
					),
					helpful_rate: (
						if .carriers_observed_total == 0 then 0
						else (.helpful_total / .carriers_observed_total)
						end
					),
					detrimental_rate: (
						if .carriers_observed_total == 0 then 0
						else (.detrimental_total / .carriers_observed_total)
						end
					),
					high_confidence_rate: (
						if .carriers_observed_total == 0 then 0
						else (.confidence_high_total / .carriers_observed_total)
						end
					),
					invalid_action_rate: (
						if .action_attempted_total == 0 then 0
						else (.invalid_action_total / .action_attempted_total)
						end
					)
				}
			)
			| map(
				. + {
					delta_vs_global: {
						...
						viability_score: (.mean_viability_score - $global.mean_viability_score)
					}
				}
			)
			| map(
				. + {
					above_global_metric_count: (
						...
						+ (if .delta_vs_global.viability_score > 0 then 1 else 0 end)
					)
				}
			)
			| sort_by(.mean_viability_score, .helpful_rate, .mean_offspring_spawned, .mean_survival_ticks, .mean_final_energy)
			| reverse
			| .[0:$top_n]
		)
	}'
