...
import type { GraphInternalNode, GraphNodeKind, InputReference } from "../../types/genome.ts";
...
export function getKindLabel(kind: GraphNodeKind, inputRefs: InputReference[]): string {
...
export function categorize(
	kind: GraphNodeKind,
): "input" | "constant" | "memory" | "processing" | "output" {
		...
		const flowNodes: Node<GraphInternalsNodeData>[] = layout.nodes.map((ln) => {
			...
			return {
				...
				data: {
					...
					selectedTarget:
						kind && getKindName(kind) === "RouterOutput" && routeTargetIdx !== null
