import type { MeshBackendFilter, MeshFocusMode } from "../../../stores/inspectorWorkspace.ts";
...
export function MeshControls({
	...
	return (
		<div className="flex items-center gap-2 border-b border-white/6 px-3 py-1">
			<span className="text-[9px] font-mono uppercase tracking-[0.16em] text-slate-500">Focus</span>
			...
			<button
				...
				className={`rounded px-2 py-0.5 text-[10px] font-mono transition-colors ${
					dimUnreachable ? "bg-white/[0.04] text-slate-300" : "text-slate-500 hover:text-slate-300"
				}`}
			>
				Dim junk
			</button>
