...
import { useSamplerControls } from "./useSamplerControls.ts";
...
describe("useSamplerControls", () => {
	...
	it("returns idle playback state when no sample exists", () => {
		const { result } = renderHook(() => useSamplerControls(1));
	...
	it("handleSample calls startSampling with creatureId", () => {
		const { result } = renderHook(() => useSamplerControls(42));
	...
	it("auto-clears sample when creatureId changes", () => {
		const { rerender } = renderHook(
			({ creatureId }: { creatureId: number | null }) => useSamplerControls(creatureId),
