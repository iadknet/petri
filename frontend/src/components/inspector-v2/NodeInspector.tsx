...
import type { MeshAnalyzedNode } from "../inspector/mesh/meshAnalysis.ts";
import { collectNodeBadges, summarizeBackendDef } from "../inspector/mesh/meshPresentation.ts";
import type { MeshNodeSemantics } from "../inspector/mesh/meshSemantics.ts";
			...
			{isGraph && "Graph" in node.node.backend_def ? (
				...
				>
					<GraphInternalsViz
						...
						inputRefs={node.node.input_refs}
						targets={node.node.targets}
						routeTargetIdx={executionHop?.hop.route_target_idx ?? null}
						trace={graphTrace}
