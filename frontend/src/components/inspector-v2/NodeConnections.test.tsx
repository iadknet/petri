...
vi.mock("../inspector/inputRefUtils.ts", () => ({
...
describe("NodeConnections", () => {
	it("renders input refs as comma-separated list", () => {
		...
		render(<NodeConnections inputRefs={inputRefs} targets={[2]} incomingCount={1} />);
	...
	it("renders target node IDs", () => {
		render(<NodeConnections inputRefs={[]} targets={[2, 4]} incomingCount={0} />);

		expect(screen.getByText("#2, #4")).toBeInTheDocument();
	});

	it("shows dash when no targets", () => {
		render(<NodeConnections inputRefs={[]} targets={[]} incomingCount={3} />);
