...
| `world.food.shared.grazing.recovery_ticks` | `u32` | `1000` | Minimum `1`; zero or missing falls back to `1000`. At the start of every Phase 0 growth pass each non-barrier cell of every type advances `min(1.0, modifier + 1 / recovery_ticks)`, empty cells included; barrier cells stay `1.0`. |
