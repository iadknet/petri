...
**Scope:** Test coverage map for `CP-0`..`CP-3`, including required suites, pass criteria, and command gates for gate-driven execution only; excludes new feature design work.
...
| Should nightly/scheduled test tracks be part of this matrix? | No; this matrix defines gate-only execution and does not require nightly/scheduled runs. | user+agent | resolved |
| Should Playwright own broad regression coverage? | No; Playwright is limited to gate-critical smoke coverage, with most coverage in unit/integration suites. | user+agent | resolved |
...
| `CP-3` | server lifecycle/payload/ws tests + `startup_world_init`, `startup_founder_phenotype`, `startup_viability`, `tick_dynamics`; core runtime realism regression (`world_state`); cli ndjson tests; web protocol fixtures + unit/integration tests; Playwright gate smoke | all tests pass; schema/version parity across all surfaces, lifecycle transition contract is enforced, non-2xx responses follow error-envelope schema, ws event/payload schemas are fixture-locked, startup world initialization is deterministic and non-empty for viable starts, running ticks mutate creature/world behavior beyond tick counters, action/energy/population telemetry reflects applied behavior (not synthetic derivation), and gate-smoke UI flows pass against real server transport |

## Gate-Only Execution Policy

1. This matrix defines required tests for checkpoint and merge gates only.
2. Nightly/scheduled suites are out of scope for this plan.
3. Unit and integration tests are the primary signal; Playwright is limited to gate-critical smoke flows.

## Functional Intent Rule (Golden Rule, Generalized)

1. This matrix must always include intent/integration coverage for each checkpoint feature area.
2. Contract-shape verification is required but never sufficient on its own.
3. Future checkpoints must include equivalent evidence for:
- functional intent behavior (what the feature is meant to do)
- integrated execution-path behavior (fully wired, not isolated stubs)
- negative/depletion/error path behavior where applicable
4. Runtime surfaces additionally require behavior-realism checks (state evolution and telemetry truthfulness).
...
3. minimal gate e2e (`server + web` connect and receive status/frame)

### E2E (Playwright, Gate-Critical Only)

1. startup draft apply and first frame visible
2. runtime controls (`start` -> `pause` -> `step`) against live server
3. paint workflow happy path (preview during drag, single commit on pointer-up)
...
5. No nightly/scheduled test cadence is required by this matrix.
...
4. `cd v2 && cargo test -p v2-core --test world_state`
5. `cd v2 && cargo test -p v2-server --test tick_dynamics`
6. `cd v2 && cargo test --workspace`
7. `cd v2/web && npm run test`
8. `cd v2/web && npm run build`
9. `cd v2/web && npm run test:e2e`
