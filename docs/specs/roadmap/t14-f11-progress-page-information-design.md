...
`docs/progress/index.html` as it stands (74,604 bytes; helpers `num`, `get`,
`card`, `addTableToggle`, `table`, `legend`, `tooltip`/`showTip`, `lineChart`,
`stackedColumns`, `heatmap`, `statTile`, `group`, `loadAll`, `validateArtifact`,
`artifactNotice`, `meshSums`, the tick-zero map card); `scripts/benchmark-artifacts.test.mjs`;
...
(generated 2026-09-16T01:35Z at `bdfb7675`), and for the grazing fields
`t02-f04-grazing-recovery-and-overuse-goal.json` (the F07 run predates its
rebase onto T02.F04, so its summary lacks them); the T14.F12 spec's floor
paragraph; the T11 track's 2026-09-14 floor amendment.
...
**Options considered.** (a) Keep the single static file with its own SVG
primitives and rewrite the information architecture — taken; the helpers are
covered by the page test and the scope note forbids a build step. (b) A
charting library from a CDN — rejected: a network dependency on a page read
from Git, and nothing the primitives lack. (c) Restructure by moving existing
cards under new headings — rejected: the per-world stacks and tabs are the
problem, and the amendment permits a wholesale rewrite that keeps the helpers.
...
- [ ] Remove the profile tabs and their localStorage state; keep `loadAll`,
...
- [ ] Implement once, and use for every card, the five behaviours below.
  - **World row.** Every indicator is one row of three panels, Orchards,
    Canyon, Confluence, sharing one y-scale; each panel keeps its JSON-path
    subtitle, table toggle and hover readout naming closure, host and date.
  - **Series break.** Charts over closure order draw `goal-v1` closures left of
    a vertical break labelled "goal-worlds-v1 from T12.F04"; the left segment is
    the same seed's default-world reading, muted and labelled "goal-v1, default
    world, seed N" in legend and hover. No delta crosses the break. A reading
    with no `goal-v1` counterpart (every per-case block) starts at the break.
  - **Not measured.** A position with no reading (T13.F02, an `Undefined`
    indicator, a pre-T02.F04 grazing sample, `absent_in_reference`) is a gap
    with a "not measured" marker, never zero.
  - **Deltas.** A percent delta shown anywhere is read from
    `comparison.references[]` — the entry whose `path` is the previous closed
    summary in the series — by reading or counter name, and is printed as
    "single run, vs T14.F07" with the `level` when the block carries one. A
    tile or chart whose name is not in that block shows no percentage.
  - **Denominators.** A fraction prints its count and denominator beside the
    value ("6 of 9,772"); a fixed-sample reading prints the sample ("5,000
    births from 50 genomes").
- [ ] Headline row, in the first screen at 1280 × 800:
...
- [ ] Sections in this order, each with an `h2` phrased as its question, a
...
- [ ] Drop the cards that fail the question test: the six per-world
      ...
      tiles.
- [ ] Run the impeccable detector once when the page is finished
...
- [ ] `make check-docs` -> exit 0, including
      `node --test scripts/benchmark-artifacts.test.mjs` against the rewritten
      page with the test file unchanged.
- [ ] Orchestrator browser check over HTTP before review
...
- [ ] Fresh `MUTANTS_ITERATE=0 make rust-mutants`: Not applicable, no Rust
      changes (amendment of 2026-09-15).
- [ ] Benchmark summary: Not applicable, the feature cannot change simulation
      cost (amendment of 2026-09-15).
...
- Decision: every percent delta on the page is read from the stored
  `comparison.references[]` block for the previous closed summary and labelled
  single-run; nothing is recomputed from two reports.
