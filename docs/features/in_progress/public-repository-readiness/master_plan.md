...
- [x] Step 1: Executed the [Root Workspace Promotion companion plan](root-workspace-promotion.md). Promoted the active Cargo workspace to root in equivalence-proven commit `9f899cd`, then declared/inherited Rust 1.93 and the minimal workspace lint policy in separately reviewed commit `50414406`. Crate/API behavior, source fingerprints, metadata, viability, all-feature workspace tests, frontend integration, and reviewer gates passed; known doc/architecture baselines did not regress. Strict-green restoration is intentionally deferred to Step 2.
