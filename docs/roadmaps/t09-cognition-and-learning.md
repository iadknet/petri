...
- A substrate gap exposed by a T09 assay (a controller that cannot represent, reach, or retain the tested behavior) is recorded and handed to T11 as one bounded feature with matched observations, actions, task exposure, and compute accounting. It is not repaired inside an assay, and no replacement brain is added here.
