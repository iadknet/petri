...
- [ ] **T08.F01 — Mutational Neighborhood Assay** — Depends on: T01.F12
  - Goal: One step away. Apply single mutations to representative genomes in process and measure how many children stay viable and how far their behavior moves; observation only.
...
- [ ] **T08.F06 — Homology and Mating Compatibility** — Depends on: T04.F05, T08.F04
  - Goal: Deferred until the go/no-go passes. Which genomes are similar enough to exchange parts, measured before any mating exists.
- [ ] **T08.F07 — Dormant Dual-Parent Mating Contract** — Depends on: T03.F07, T08.F06
  - Goal: Deferred until the go/no-go passes. Two-parent reproduction semantics, present but disabled by default.
- [ ] **T08.F08 — Homologous Recombination and Sexual-Mode Enablement** — Depends on: T08.F07
  - Goal: Deferred until the go/no-go passes. Turn on sex only after recombination provably exchanges homologous function.
...
- T08.F01 runs in process on genomes sampled from a goal-profile run: apply each operator to each sample, run the child on fixed sensor snapshots, and compare viability and action agreement with the parent. It needs no replay mechanics, saved specimens, or reference controllers, so it depends on T01.F12 only. Measure local mutational neighborhoods before tuning operator probabilities. Separate neutral structure that improves robustness from unreachable bloat that only absorbs mutations.
...
- T08.F06 through T08.F09 are deferrable: do not plan their specs until the T09.F08 go/no-go in the master roadmap has passed and T08.F01 through T08.F05 have produced mutational-neighborhood evidence.
