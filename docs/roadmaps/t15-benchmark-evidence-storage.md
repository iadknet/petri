...
- [x] All future full gate, goal and sweep artifacts default to the main
...
- [x] Committed, versioned summaries occupy the existing
...
- [x] Comparison loading accepts both historical full reports and new summaries
...
- [x] Every full benchmark report reachable from the pre-migration `main` is
  ...
  dropped.
- [x] Full benchmark report paths and blobs are absent from rewritten `main`
  ...
  can erase. This candidate's checked state becomes effective only after the
  authorization, leased push and external postpush receipt required by
  [T15.F02's staging condition](../specs/roadmap/t15-f02-historical-report-migration-and-main-history-rewrite.md#inputs-and-invariants).
- [x] The shared workflow, Codex adapter, feature template, output conventions
...
- [x] **T15.F02 — Historical Report Migration and Main-History Rewrite** — Depends on: T15.F01
...
- Decision: Added at the user's direction on 2026-09-12 after T13.F02's single
  ...
  T15.F02 is staged Complete, conditional on its external cutover receipt, under its
