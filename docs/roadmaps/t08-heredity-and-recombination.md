...
- [ ] **T08.F06 — Homology and Mating Compatibility** — Depends on: T11.F08, T08.F05, T09.F08
...
- T08.F05 begins with an inherited scalar mutation rate, with bounds informed and justified by T11.F13 and fixed-rate controls from that characterization (user decision, 2026-09-05). The inherited scalar is T11.F19's `mutation.per_unit_rate`, the per-`genome_size()`-unit copy-error rate whose default of 0.005 is provisional, not a minimum or a claimed optimum. The 111-unit founder and roughly 0.55 requested events per birth were its historical calibration; compute current founder supply from the canonical founder’s asserted size, which T19.F04 changed; a lineage's realized supply is its inherited rate times its genome size, so the policy is read beside T03.F08's carrying cost and T03.F11's replication cost, the bounds on that load (T11.F19 replaced the production rule; T11.F20 still owns retirement of the disabled legacy rule). Resolve insufficient evidence for bounds explicitly before enabling inherited policy; never invent a floor from the existing default. [Clune et al. (2008)](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1000187) found that evolved rates could fall below the long-term adaptation optimum, while [Kumawat et al. (2025)](https://pubmed.ncbi.nlm.nih.gov/39739809/) found that environmental change could support elevated rates and useful alternate phenotypes. Evolved rates therefore need measured discovery/retention and supply outcomes, not an assumption of self-optimization. Lifetime responses to stress are a separate, unscheduled policy, and T08.F05 adds no new mutation operators.
...
- F06's bounded compatibility, viability, function-retention and cost comparisons do not require T04.F05's deferred transplant/invasion assay or its campaign infrastructure. That prerequisite is removed under the master rule separating mechanisms from the proof phase; F06–F08 remain unscheduled later work, and F09 retains the confirmatory dependencies.
