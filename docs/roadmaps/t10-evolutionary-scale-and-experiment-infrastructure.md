...
- T10.F09 measures ticks per second and per-tick cost at every T01.F11 sweep point through the T10.F10 sweep profile, profiles the tick hot paths, and records a per-core budget expressed in births and ticks per wall-clock hour. It decides, with numbers, whether core parallelism is required before any campaign; if so it adds one bounded dependency-linked feature rather than implementing parallelism itself. Measured 2026-09-04 in the eight reports under `docs/progress/sweeps/t10-f09/`, the default 1600-by-1600 world with 10,000 founders ran at 9.53 ticks per second on one thread and 10.54 on all eight logical cores, because Phase 1b cognition already runs on the rayon global pool while Phase 0, sensor assembly, actions, and reward learning are all sequential and Phase 0 alone costs about 59 milliseconds per tick at that world size. For T10.F04: on the recording host the eight-thread speedup (1.106) beats eight concurrent one-thread replicates (0.867 lone-run equivalents, measured), so campaigns should run replicates in sequence on all cores rather than one replicate per core. The scale reference in the master roadmap is roughly 190 steps per second with 60,000 agents on a GPU.
