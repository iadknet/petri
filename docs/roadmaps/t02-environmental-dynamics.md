...
- T02.F01 and T02.F02 are the first mechanism features in the execution order. Each spec defines one production-world condition that runs on the T01.F12 goal profile within its budget and is verified there before any richer composition is added. Both must show no severe compute regression on the gate profile and record their goal-profile indicator readings at closure.
